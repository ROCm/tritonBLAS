    ...
    for m, n, k, in_dtype, out_dtype, transA, transB in (
        ...
        if inputs.is_quantized:
            ...
            matmul = lambda: tritonblas.matmul_lt(
                ...
                inputs.activeCUs, inputs.numCUs,
                selector, cfg,
                enable_streamk, work_stealing=work_stealing,
            )

        cus_to_release=total_cus-active_cus
        print(f"cus_to_release: {cus_to_release}", file=sys.stderr)
        reset = lambda: cfg.reset(streamk=enable_streamk, work_stealing=work_stealing)
        ms = tritonblas.do_bench_release(matmul, inputs.numCUs_hptr, cus_to_release=cus_to_release, reset_fn=reset, n_warmup=20, n_repeat=100)
        perf = gflops(ms)
