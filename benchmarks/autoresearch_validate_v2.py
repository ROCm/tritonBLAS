#!/usr/bin/env python3
"""
Autoresearch: Combined validation (gm=8 + adaptive split).
"""
...
def bench_ws(sz):
    ...
    print(f"    Config: gm={gsize_m}, n_cu={n_cu}, xcds={num_xcds}, "
          f"tiles={total_tiles}, tiles/cu={tiles_per_cu:.1f}, "
          f"local/xcd={local_per_xcd}, global={global_tiles}, "
          f"split={'100/0' if global_tiles == 0 else f'{local_per_xcd*num_xcds*100//total_tiles}/{global_tiles*100//total_tiles}'}")
...
if __name__ == "__main__":
    print("=" * 80)
    print("  Combined Validation: gm=8 + adaptive split (Phase 4+5)")
    print("=" * 80)
    ...
    for sz in [4096, 8192, 12288, 16384]:
        ...
        results[str(sz)] = {
            "torch_ms": torch_ms, "torch_tf": torch_tf,
            "ws_ms": ws_ms, "ws_tf": ws_tf, "gap_pct": gap,
        }

    print(f"\n{'='*80}")
    print("  Summary: Raw GEMM gap progression")
    print(f"{'='*80}")
    print(f"  {'Size':>6s}  {'Original':>10s}  {'Phase 4':>10s}  {'Phase 5':>10s}")
    original = {"4096": 28, "8192": 14, "12288": 10.5, "16384": 8.9}
    phase4   = {"4096": 14.2, "8192": 11.2, "12288": 8.9, "16384": 7.6}
    for k, r in results.items():
        o = original.get(k, "?"); p4 = phase4.get(k, "?")
        print(f"  {k:>6s}  {o:>+9.1f}%  {p4:>+9.1f}%  {r['gap_pct']:>+9.1f}%")
    print()
