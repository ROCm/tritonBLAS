...
Usage:
    python tools/fp16_selector_efficiency_sweep.py
    python tools/fp16_selector_efficiency_sweep.py --sizes small
    python tools/fp16_selector_efficiency_sweep.py --output results.csv
"""
...
def benchmark_config(a, b, M, N, K,
    ...
    # Create selector for this specific configuration
    selector = tritonblas.OrigamiMatmulSelector(M, N, K, dtype, dtype, dtype, a.device)

    # Override selector configuration with our test values
    # We have to modify the internal result object
    selector._result.config.mt.m = block_m
    selector._result.config.mt.n = block_n
    selector._result.config.mt.k = block_k
    selector._workgroup_mapping = 8  # Standard group size

    # Create config once outside the benchmark function
    config = tritonblas.matmul_preamble(selector)

    # Define the function to benchmark
    def run_kernel():
        # matmul_lt signature: matmul_lt(a, b, c, selector, config, enable_streamk, work_stealing)
        tritonblas.matmul_lt(a, b, out, selector, config, enable_streamk=False, work_stealing=False)
...
def exhaustive_search(a, b, M, N, K, dtype=torch.float16):
    """
    ...
    block_k_range = [64, 128, 256]  # FP16 doesn't have the same constraints as FP4
    num_warps_range = [4, 8]
                ...
                for num_warps in num_warps_range:
                    for num_stages in num_stages_range:
                        ...
                        time_us, error = benchmark_config(
                            a, b, M, N, K,
                            block_m, block_n, block_k, num_warps, num_stages, dtype
                        )
...
def test_selector_vs_exhaustive(M, N, K, dtype=torch.float16):
    """
    ...
    # Create selector to trigger it
    selector = tritonblas.OrigamiMatmulSelector(M, N, K, dtype, dtype, dtype, a.device)
    selector_time = (time.time() - selector_start) * 1e6

    # Get selector's choice by accessing the selector object
    sel_m = selector.block_m
    sel_n = selector.block_n
    sel_k = selector.block_k
    sel_gsize = selector.group_m
...
def main():
    parser = argparse.ArgumentParser(
        description="FP16 GEMM Selector Efficiency Sweep",
    ...
    print("\n" + "="*80)
    print("FP16 GEMM Selector Efficiency Sweep")
    print("="*80)
