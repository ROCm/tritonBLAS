...
class InputTensorA:
    """
    Input tensor A descriptor for GEMM.
    """
    ptr: tl.tensor
    stride_am: tl.tensor
    ...
    @triton.constexpr_function
    def __init__(self, ptr, stride_am, stride_ak, M, K):
        """
        Create InputTensorA.
        
        Args:
            ptr: Base pointer to matrix A
            stride_am: Stride in M dimension (runtime)
            stride_ak: Stride in K dimension (constexpr)
            M: Number of rows
            K: Number of columns
        """
        self.ptr = ptr
        self.stride_am = stride_am
        self.stride_ak = tl.constexpr(stride_ak)
        self.M = M
        self.K = K
...
class InputTensorB:
    """
    Input tensor B descriptor for GEMM.
    """
    ptr: tl.tensor
    ...
    @triton.constexpr_function
    def __init__(self, ptr, stride_bk, stride_bn, K, N):
        """
        Create InputTensorB.
        
        Args:
            ptr: Base pointer to matrix B
            stride_bk: Stride in K dimension (constexpr)
            stride_bn: Stride in N dimension (runtime)
            K: Number of rows
            N: Number of columns
        """
        self.ptr = ptr
        self.stride_bk = tl.constexpr(stride_bk)
        self.stride_bn = stride_bn
        self.K = K
        self.N = N